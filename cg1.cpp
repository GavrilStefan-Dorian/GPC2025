...
double g_jfa = -0.81, g_jfb = -0.20; //Julia-Fatou a and b values.
...
void drawCarpetFractal(Turtle t, float distance, int recursionsLeft = 1) {
    if (recursionsLeft > 1) {
        --recursionsLeft;
        drawSquare(t, distance);
        std::vector<Turtle> turtles(8, t);

        auto angle = pi / 4;
        for (int i = 0; i < turtles.size(); ++i) {
            turtles[i].rotate(angle * i);
            if (i % 2 == 0) {
                turtles[i].move(distance);
            }
            else {
                turtles[i].move(sqrt(2 * distance * distance));
            }
            turtles[i].rotate(-angle * i);
        }
